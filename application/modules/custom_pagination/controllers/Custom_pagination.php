...
class Custom_pagination extends MX_Controller 
{
	...
	{	
		...
		if ($template=="public") {
			$settings = $this->get_settings_for_public();
	...
	public function _get_showing_statement($data)
	{
	...
	public function get_settings_for_public()
	{
		$settings['num_links'] = 12;
		$settings['full_tag_open'] = '<nav class="pagination is-rounded" role="navigation" aria-label="pagination"><ul class="pagination-list">';
		$settings['full_tag_close'] = '</ul></nav>';

		$settings['cur_tag_open'] = '<li><a href="#" class="pagination-link is-current" aria-label="Page 46" aria-current="page">';
		...
		$settings['attributes'] = array('class' => 'pagination-link');
		return $settings;
	}
